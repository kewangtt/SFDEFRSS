	...
	public:
		...
		*/
		struct Parameters
		{
			...
			*/
			Parameters(int P1 = 10, int P2 = 120, float uniqueness = 0.95f, bool subpixel = true, PathType path_type = PathType::SCAN_8PATH, int min_disp = 0, int LR_max_diff = 1)
				: P1(P1)
				, P2(P2)
		...
		*/
		LIBSGM_API StereoSGM(int width, int height, int disparity_size, int input_depth_bits, int output_depth_bits, 
			EXECUTE_INOUT inout_type, int min_disparity = 0, bool is_subpixel = false, const Parameters& param = Parameters());
		...
		*/
		...
		// Split the main interface to subinterface
		// 1. Census transform
		// 2. Compute costvolumes
		// 3. DP for smooth terms
		// 4. Winner take all
		// 5. Post process 

		/**
		* left_dst and right_dst, must be cuda memory
		*/
		LIBSGM_API void CensusTransform(const unsigned char* left_pixels, const unsigned char* right_pixels, unsigned int ** left_dst, unsigned int ** right_dst);
		
		/**
		* left_dst and right_dst, must be cuda memory
		*/ 
		LIBSGM_API void InvolveSmooth(unsigned int* cost_volume, const void* left_pixels, void* dst);
		LIBSGM_API void InvolveSmooth2(unsigned int* cost_volume, int width, int height, void * finalcostCpu);
