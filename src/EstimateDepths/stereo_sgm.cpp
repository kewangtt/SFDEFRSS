...
namespace sgm {
	...
	public:
		...
		virtual void CensusTransform(const uint8_t *src_left,
			const uint8_t *src_right,
			feature_type ** left_out,
			feature_type ** right_out,
			int width,
			int height){}
		virtual void InvolveSmooth(output_type * dst_L, output_type * dst_R, const uint32_t * cost_volume, int w, int h, StereoSGM::Parameters& param){}
		virtual void InvolveSmooth2(uint8_t ** finalcost, const uint32_t * cost_volume, int w, int h, StereoSGM::Parameters& param){}
	};
	...
	public:
		...
		void CensusTransform(const uint8_t *src_left,
			const uint8_t *src_right,
			feature_type ** left_out,
			feature_type ** right_out,
			int width,
			int height)
		{
			sgm_engine_.FeatureTransform((input_type *)src_left, (input_type *)src_right, width, height, width);
			*left_out = (feature_type *)(sgm_engine_.get_left_output());
			*right_out = (feature_type *)(sgm_engine_.get_right_output());
		}


		void InvolveSmooth(output_type * dst_L, output_type * dst_R, const uint32_t * cost_volume, int w, int h, StereoSGM::Parameters& param)
		{
			sgm_engine_.InvolveSmooth(dst_L, dst_R, cost_volume, w, h, param);
		}

		void InvolveSmooth2(uint8_t ** finalcost, const uint32_t * cost_volume, int w, int h, StereoSGM::Parameters& param)
		{
			sgm_engine_.InvolveSmooth2(finalcost, cost_volume, w, h, param);
		}
	...
	StereoSGM::StereoSGM(int width, int height, int disparity_size, int input_depth_bits, int output_depth_bits,
		EXECUTE_INOUT inout_type, int min_disparity, bool is_subpixel, const Parameters& param) : StereoSGM(width, height, disparity_size, input_depth_bits, output_depth_bits, width, width, inout_type, min_disparity, is_subpixel, param) {}
	...
	StereoSGM::StereoSGM(int width, int height, int disparity_size, int input_depth_bits, int output_depth_bits, int src_pitch, int dst_pitch,
		EXECUTE_INOUT inout_type, int min_disparity, bool is_subpixel, const Parameters& param) :
		cu_res_(NULL),
		width_(width),
		height_(height),
		...
		param_(param)
	{
		...
		param_.min_disp = min_disparity;
		printf("Min disparity is %d\n", param_.min_disp);

		param_.subpixel = is_subpixel;		
		if (is_subpixel){
			printf("Is_subpixel: yes!!!\n");
		}
		else{
			printf("Is_subpixel: No!!!\n");
		}
		...
		// Create memory managment object
	...
	int StereoSGM::get_invalid_disparity() const {
		printf("StereoSGM: get_invalid_disparity %d\n", (param_.min_disp - 1) * (param_.subpixel ? SUBPIXEL_SCALE : 1));
		return (param_.min_disp - 1) * (param_.subpixel ? SUBPIXEL_SCALE : 1);
	}

	void StereoSGM::CensusTransform(const uint8_t* left_pixels, const uint8_t* right_pixels, feature_type ** left_out, feature_type ** right_out){
		cu_res_->sgm_engine->CensusTransform(left_pixels, right_pixels, left_out, right_out, width_, height_);
	}

	void StereoSGM::InvolveSmooth(unsigned int* cost_volume, const void* left_pixels, void* dst){

		const void *d_input_left;
		if (is_cuda_input(inout_type_)) {
			d_input_left = left_pixels;
		}
		else {
		...
		cu_res_->sgm_engine->InvolveSmooth((uint16_t*)d_tmp_left_disp, (uint16_t*)d_tmp_right_disp, cost_volume, width_, 
		height_, param_);
		...
		sgm::details::check_consistency_rs((uint16_t*)d_left_disp, (uint16_t*)d_right_disp, d_input_left, width_, height_, input_depth_bits_, src_pitch_, dst_pitch_, param_.subpixel, param_.LR_max_diff);
	...
	void StereoSGM::InvolveSmooth2(unsigned int* cost_volume, int width, int height, void * finalcostCpu){
		
		uint8_t * finalcost;
		cu_res_->sgm_engine->InvolveSmooth2(&finalcost, cost_volume, width, height, param_);

		CudaSafeCall(cudaMemcpy(finalcostCpu, finalcost, 8 * sizeof(uint8_t) * width * height * disparity_size_, cudaMemcpyDeviceToHost));
