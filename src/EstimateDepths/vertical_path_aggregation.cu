...
template <int DIRECTION, unsigned int MAX_DISPARITY>
__global__ void smooth_vertical_path_kernel(
	uint8_t *dest,
	...
	int min_disp)
{
	...
	for(unsigned int iter = 0; iter < height; ++iter){
		...
		const unsigned int DisparityAddr = (x + y * width)*MAX_DISPARITY + dp_offset;

		if(x < width){
			uint32_t local_costs[DP_BLOCK_SIZE];
			for(unsigned int j = 0; j < DP_BLOCK_SIZE; ++j){
				local_costs[j] = __popc(costs[DisparityAddr + j]);
			}
			// Propagate data costs with smooth term in a line
			dp.update(local_costs, p1, p2, shfl_mask);
			// Store 
			store_uint8_vector<DP_BLOCK_SIZE>(
				&dest[dp_offset + x * MAX_DISPARITY + y * MAX_DISPARITY * width],
...
template <unsigned int MAX_DISPARITY>
void enqueue_smooth_up2down_path(
	cost_type *dest,
	const uint32_t * costs,
	int width,
	int height,
	...
	cudaStream_t stream)
{
	...
	const int gdim = (width + PATHS_PER_BLOCK - 1) / PATHS_PER_BLOCK;
	const int bdim = BLOCK_SIZE;
	smooth_vertical_path_kernel<1, MAX_DISPARITY><<<gdim, bdim, 0, stream>>>(
		dest, costs, width, height, p1, p2, min_disp);
}

template <unsigned int MAX_DISPARITY>
void enqueue_smooth_down2up_path(
	cost_type *dest,
	const uint32_t * costs,
	int width,
	int height,
...
template void enqueue_smooth_up2down_path<64u>(
	cost_type *dest,
	const uint32_t * costs,
	int width,
	int height,
...
template void enqueue_smooth_up2down_path<128u>(
	cost_type *dest,
	const uint32_t * costs,
	int width,
	int height,
...
template void enqueue_smooth_up2down_path<256u>(
	cost_type *dest,
	const uint32_t * costs,
	int width,
	int height,
...
template void enqueue_smooth_down2up_path<64u>(
	cost_type *dest,
	const uint32_t * costs,
	int width,
	int height,
...
template void enqueue_smooth_down2up_path<128u>(
	cost_type *dest,
	const uint32_t * costs,
	int width,
	int height,
...
template void enqueue_smooth_down2up_path<256u>(
	cost_type *dest,
	const uint32_t * costs,
	int width,
	int height,
