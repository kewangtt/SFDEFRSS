...
template <int DIRECTION, unsigned int MAX_DISPARITY>
__global__ void smooth_horizontal_path_kernel(
	uint8_t *dest,
	const uint32_t *cost,
	int width,
	int height,
	...
	for(unsigned int iter = 0; iter < width; iter += DP_BLOCK_SIZE){
		for(unsigned int i = 0; i < DP_BLOCK_SIZE; ++i){
			...
			for(unsigned int j = 0; j < DP_BLOCKS_PER_THREAD; ++j){
				...
				uint32_t local_costs[DP_BLOCK_SIZE];
				for(unsigned int k = 0; k < DP_BLOCK_SIZE; ++k){
					local_costs[k] = __popc(cost[j * dest_step + x*MAX_DISPARITY + dp_offset + k]);
				}
				dp[j].update(local_costs, p1, p2, shfl_mask);
				store_uint8_vector<DP_BLOCK_SIZE>(
					&dest[j * dest_step + x * MAX_DISPARITY + dp_offset],
...
template <unsigned int MAX_DISPARITY>
void enqueue_smooth_left2right_path(
	cost_type *dest,
	...
	cudaStream_t stream)
{
	...
	const int gdim = (height + PATHS_PER_BLOCK - 1) / PATHS_PER_BLOCK;
	const int bdim = BLOCK_SIZE;
	smooth_horizontal_path_kernel<1, MAX_DISPARITY><<<gdim, bdim, 0, stream>>>(
		dest, costs, width, height, p1, p2, min_disp);
}

template <unsigned int MAX_DISPARITY>
void enqueue_smooth_right2left_path(
	cost_type *dest,
	const uint32_t * costs,
	int width,
	int height,
...
template void enqueue_smooth_left2right_path<64u>(
	cost_type *dest,
	const uint32_t * costs,
	int width,
	int height,
...
template void enqueue_smooth_left2right_path<128u>(
	cost_type *dest,
	const uint32_t * costs,
	int width,
	int height,
...
template void enqueue_smooth_left2right_path<256u>(
	cost_type *dest,
	const uint32_t * costs,
	int width,
	int height,
...
template void enqueue_smooth_right2left_path<64u>(
	cost_type *dest,
	const uint32_t * costs,
	int width,
	int height,
...
template void enqueue_smooth_right2left_path<128u>(
	cost_type *dest,
	const uint32_t * costs,
	int width,
	int height,
...
template void enqueue_smooth_right2left_path<256u>(
	cost_type *dest,
	const uint32_t * costs,
	int width,
	int height,
