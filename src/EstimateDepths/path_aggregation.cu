	...
	cudaStream_t stream)
{
	...
	if(m_cost_buffer.size() < buffer_size){
		if (m_cost_buffer.size() > 0) m_cost_buffer.destroy();
...
template <size_t MAX_DISPARITY>
void PathAggregation<MAX_DISPARITY>::enqueue2(
	const uint32_t *cost,
	int width,
	int height,
	PathType path_type,
	...
	cudaStream_t stream)
{
	...
	path_aggregation::enqueue_smooth_up2down_path<MAX_DISPARITY>(
		m_cost_buffer.data() + 0 * buffer_step,
		cost, width, height, p1, p2, min_disp, m_streams[0]);
	path_aggregation::enqueue_smooth_down2up_path<MAX_DISPARITY>(
		m_cost_buffer.data() + 1 * buffer_step,
		cost, width, height, p1, p2, min_disp, m_streams[1]);
	path_aggregation::enqueue_smooth_left2right_path<MAX_DISPARITY>(
		m_cost_buffer.data() + 2 * buffer_step,
		cost, width, height, p1, p2, min_disp, m_streams[2]);
	path_aggregation::enqueue_smooth_right2left_path<MAX_DISPARITY>(
		m_cost_buffer.data() + 3 * buffer_step,
		cost, width, height, p1, p2, min_disp, m_streams[3]);

	if (path_type == PathType::SCAN_8PATH) {
		path_aggregation::enqueue_smooth_upleft2downright_path<MAX_DISPARITY>(
			m_cost_buffer.data() + 4 * buffer_step,
			cost, width, height, p1, p2, min_disp, m_streams[4]);
		path_aggregation::enqueue_smooth_upright2downleft_path<MAX_DISPARITY>(
			m_cost_buffer.data() + 5 * buffer_step,
			cost, width, height, p1, p2, min_disp, m_streams[5]);
		path_aggregation::enqueue_smooth_downright2upleft_path<MAX_DISPARITY>(
			m_cost_buffer.data() + 6 * buffer_step,
			cost, width, height, p1, p2, min_disp, m_streams[6]);
		path_aggregation::enqueue_smooth_downleft2upright_path<MAX_DISPARITY>(
			m_cost_buffer.data() + 7 * buffer_step,
			cost, width, height, p1, p2, min_disp, m_streams[7]);
	}
