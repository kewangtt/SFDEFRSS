...
template <int X_DIRECTION, int Y_DIRECTION, unsigned int MAX_DISPARITY>
__global__ void smooth_oblique_path_kernel(
	uint8_t *dest,
	const uint32_t *cost,
	int width,
	int height,
	...
	int min_disp)
{
	...
	for(unsigned int iter = 0; iter < height; ++iter){
		...
		if(0 <= x && x < static_cast<int>(width)){
			uint32_t local_costs[DP_BLOCK_SIZE];
			for(unsigned int j = 0; j < DP_BLOCK_SIZE; ++j){
				local_costs[j] = __popc(cost[dp_offset + x * MAX_DISPARITY + y * MAX_DISPARITY * width + j]);
			}
			dp.update(local_costs, p1, p2, shfl_mask);
			store_uint8_vector<DP_BLOCK_SIZE>(
				&dest[dp_offset + x * MAX_DISPARITY + y * MAX_DISPARITY * width],
...
template <unsigned int MAX_DISPARITY>
void enqueue_smooth_upleft2downright_path(
	cost_type *dest,
	const uint32_t *cost,
	int width,
	int height,
	...
	cudaStream_t stream)
{
	...
	const int gdim = (width + height + PATHS_PER_BLOCK - 2) / PATHS_PER_BLOCK;
	const int bdim = BLOCK_SIZE;
	smooth_oblique_path_kernel<1, 1, MAX_DISPARITY><<<gdim, bdim, 0, stream>>>(
		dest, cost, width, height, p1, p2, min_disp);
}

template <unsigned int MAX_DISPARITY>
void enqueue_smooth_upright2downleft_path(
	cost_type *dest,
	const uint32_t *cost,
	int width,
	int height,
...
template <unsigned int MAX_DISPARITY>
void enqueue_smooth_downright2upleft_path(
	cost_type *dest,
	const uint32_t *cost,
	int width,
	int height,
...
template <unsigned int MAX_DISPARITY>
void enqueue_smooth_downleft2upright_path(
	cost_type *dest,
	const uint32_t *cost,
	int width,
	int height,
...
template void enqueue_smooth_upleft2downright_path<64u>(
	cost_type *dest,
	const uint32_t *cost,
	int width,
	int height,
...
template void enqueue_smooth_upleft2downright_path<128u>(
	cost_type *dest,
	const uint32_t *cost,
	int width,
	int height,
...
template void enqueue_smooth_upleft2downright_path<256u>(
	cost_type *dest,
	const uint32_t *cost,
	int width,
	int height,
...
template void enqueue_smooth_upright2downleft_path<64u>(
	cost_type *dest,
	const uint32_t *cost,
	int width,
	int height,
...
template void enqueue_smooth_upright2downleft_path<128u>(
	cost_type *dest,
	const uint32_t *cost,
	int width,
	int height,
...
template void enqueue_smooth_upright2downleft_path<256u>(
	cost_type *dest,
	const uint32_t *cost,
	int width,
	int height,
...
template void enqueue_smooth_downright2upleft_path<64u>(
	cost_type *dest,
	const uint32_t *cost,
	int width,
	int height,
...
template void enqueue_smooth_downright2upleft_path<128u>(
	cost_type *dest,
	const uint32_t *cost,
	int width,
	int height,
...
template void enqueue_smooth_downright2upleft_path<256u>(
	cost_type *dest,
	const uint32_t *cost,
	int width,
	int height,
...
template void enqueue_smooth_downleft2upright_path<64u>(
	cost_type *dest,
	const uint32_t *cost,
	int width,
	int height,
...
template void enqueue_smooth_downleft2upright_path<128u>(
	cost_type *dest,
	const uint32_t *cost,
	int width,
	int height,
...
template void enqueue_smooth_downleft2upright_path<256u>(
	cost_type *dest,
	const uint32_t *cost,
	int width,
	int height,
