...
void PixelSelector::makeHists(const FramePym* const fh)
{
	gradHistFrame = fh;
	...
	for(int y=0;y<h32;y++)
		for(int x=0;x<w32;x++)
		{
			...
			for(int j=0;j<32;j++) for(int i=0;i<32;i++)
			{
				...
				int g = sqrtf(map0[i+j*w]);  // g is gradient 
				if(g>48) g=48; 
				hist0[g+1]++;  
				hist0[0]++;    
			}
...
Eigen::Vector3i PixelSelector::select(const FramePym* const fh,
		float* map_out, int pot, float thFactor)
{
	...
	float dw1 = setting_gradDownweightPerLevel;   // down weight
	float dw2 = dw1*dw1;      // 0.75*0.75


	int n3=0, n2=0, n4=0;
	for(int y4=0;y4<h;y4+=(4*pot)) for(int x4=0;x4<w;x4+=(4*pot))
	{
		// 4*pot = 12; segment is 4*pot
